...
class TestLoincApiClientSearch:
    ...
    def test_all_ef_fields_present_in_result(self):
        ...
        expected_fields = [
            'LOINC_NUM', 'SHORTNAME', 'LONG_COMMON_NAME', 'RELATEDNAMES2',
            'PROPERTY', 'METHOD_TYP', 'AnswerLists', 'units', 'datatype',
            'isCopyrighted', 'containsCopyrighted', 'CONSUMER_NAME', 'COMPONENT',
    ...
    def test_network_error_returns_error_entry(self):
        with patch('services.loinc_api.requests.get', side_effect=Exception('timeout')):
            results = LoincApiClient().search('hba1c')
