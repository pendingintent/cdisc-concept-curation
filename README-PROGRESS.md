...
| NCIt/BC Alignment          | ✅ Complete    | Background job runs the `cdisc-bc-ncit-alignment` submodule's 2-stage pipeline, `services/alignment_runner.py` |
...
- Found and fixed a real bug on the way: `config.py` never called `load_dotenv()` despite `python-dotenv` being a pinned dependency, so a `.env` file was silently ignored by every entry point (`python app.py`, the MCP server, tests). Fixed by adding `load_dotenv()` at the top of `config.py`, covered by a new `tests/test_config.py`.
- `install.py` (stdlib-only, no git required) + platform wrappers `install.sh`, `Install (Mac).command`, `Install (Windows).bat` — creates `.venv`, installs `requirements.txt`, sanity-checks the `cdisc-bc-ncit-alignment` submodule isn't empty (non-fatal warning if so), prompts interactively for `CDISC_API_KEY` (required) and optional LOINC creds, generates a random `SECRET_KEY`, writes `.env`, bootstraps the DB via the same `db_bootstrap.ensure_db()` `app.py` already calls, and generates a platform `Start` launcher (`Start.bat`/`Start.command`/`start.sh`) that opens the browser automatically once the server is up.
- The `cdisc-bc-ncit-alignment` git submodule doesn't survive a plain zip/`git archive` (submodules are gitlink pointers, not fetched content) — a naive "Download ZIP" would silently break the NCIt alignment feature. Added `scripts/build_release.py`, which uses `git ls-files --recurse-submodules` (lists tracked files across the main repo and the populated submodule together, no separate ignore list needed) to stage and package both a `.zip` and `.tar.gz`, explicitly preserving the executable bit on `.sh`/`.command` files in the zip via `ZipInfo.external_attr`.
- `.github/workflows/release.yml` — new workflow, triggers on a `v*.*.*` tag push or manual `workflow_dispatch`; checks out with `submodules: recursive` (the actual fix for the gitlink problem — the packaging script only works correctly once the submodule is populated), runs `build_release.py`, and publishes both archives to a GitHub Release (`gh release create`) on a tag push, or uploads them as workflow artifacts for a manual test dispatch.
- `.env.example` (new) documents every env var actually read in code, including several the README's table had omitted (`CDISC_SUBSCRIPTION_KEY`, `PORT`, `FLASK_DEBUG`, `ALIGNMENT_SUBMODULE_DIR`) — added to the README's table too. Had to add `!.env.example` to `.gitignore`, since the existing `.env.*` ignore pattern was also swallowing the example file.
- README.md — new "Quick Start (no git required)" section pointing non-technical users at the Releases page and the installer; fixed the placeholder `your-org` clone URL to the real `pendingintent` org. New `RELEASING.md` documents the release process for maintainers.
- Verified end-to-end rather than just linted: built both archive formats locally from this checkout (using `git add -N` to stage the new files for the dry run, then unstaged), extracted each, confirmed the submodule content and executable bits survive and no `.git` directory exists anywhere in either archive, then ran `install.py` for real against the extracted zip with a throwaway API key in a scratch directory — `.venv` created, dependencies installed, `.env` written, all 9 tables created via Alembic migrations, `Start.command` launched the app and served HTTP 200 on `localhost:8081`. Not verified: an actual Windows run (no Windows machine available) — flagged as a follow-up.
- 473 tests passing (472 + 1 new `test_config.py` test), isort/black/flake8 clean ✅

#### Renamed and Repositioned the "Specializations" Sidebar Nav Item

- Renamed the "Specializations" sidebar link to "SDTM Specializations" and moved it to sit directly under "Biomedical Concepts" (previously between "Biomedical Concepts" and "NCIT Mapping", now above "NCIT Mapping"); `templates/base.html` only, no route/endpoint changes.
- All 14 smoke checks still pass.

#### Added the cdisc-bc-ncit-alignment Submodule and a Background Alignment Run Page (Issue #60)

- [Issue #60](https://github.com/pendingintent/cdisc-concept-curation/issues/60) asked for two things: incorporate the `cdisc-bc-ncit-alignment` repo as a submodule, and let the workbench run its NCIt↔CDISC BC mapping report as a background process with progress tracking.
- Added `cdisc-bc-ncit-alignment` as a git submodule (`.gitmodules`). Reviewed the submodule and found it had no `.gitignore` (generated artifacts and `__pycache__` were committed, ~65MB of tracked binary xlsx), no `requirements.txt`, an unused `throttle()` helper, and no tests for its two CLI entry points — fixed all of it on the submodule's own `fix-repo-hygiene` branch (merged as [pendingintent/cdisc-bc-ncit-alignment#4](https://github.com/pendingintent/cdisc-bc-ncit-alignment/pull/4)) before pointing this repo's gitlink at the merged commit ([#70](https://github.com/pendingintent/cdisc-concept-curation/pull/70), merged to `release-v0.5`).
- README.md — new "Submodules" section documenting `git submodule update --init` / `--remote`, and the submodule listed in the Project Structure tree.
- `models/alignment.py` — new `AlignmentJob` model tracking one run of the submodule's two-stage pipeline: `status` (`pending → running_populate → running_augment → generating_json → completed/failed`), per-stage progress counters, output file paths, timestamps, `to_dict()`. New Alembic revision `fb135e287995_add_alignment_jobs_table.py` (hand-written, not autogenerated — autogenerate missed the new table entirely since the model wasn't imported anywhere the migration env traverses yet, and instead picked up an unrelated pre-existing schema drift; tested upgrade + downgrade against a scratch DB).
- `services/alignment_runner.py` — `start_job()` guards against a concurrent run (service-layer check, not a DB constraint — this repo has no precedent for partial-unique constraints and the rigor level matches unlocked governance-stage advances elsewhere) and spawns a daemon `threading.Thread`. The thread pushes its own Flask app context and runs the submodule's `populate_complete_list` then `augment_cdisc` CLI stages as `subprocess.Popen` calls (not `subprocess.run` — stderr must be read incrementally, while the process is still running, to update live progress), parsing their stderr for batch/row/hit counts; a non-zero exit or exception on stage 1 short-circuits stage 2 and marks the job failed. `CDISC_API_KEY` is passed explicitly via the subprocess `env=` (rather than relying on inherited process env) since this app prefers `CDISC_SUBSCRIPTION_KEY` for its own CDISC client but the submodule's client only reads `CDISC_API_KEY`. On success, derives a self-describing JSON export (list of row-dicts keyed by the xlsx header row) from the final xlsx.
- `routes/ncit_alignment.py` (new blueprint, `/ncit-alignment`) — `GET /` page, `POST /start`, `GET /status` (JSON polling endpoint; never exposes raw file paths, only computed `xlsx_ready`/`json_ready` booleans), `GET /download/xlsx|json/<job_id>` using `flask.send_file` — the first use of `send_file` in this app, a deliberate deviation from every other export route's in-memory `Response` pattern, since the ~212k-row xlsx is streamed straight to disk by the subprocess (that's the whole point of its `openpyxl` write-only mode) and there's no in-memory buffer to hand back.
- `templates/ncit_alignment.html` (new) + `templates/base.html` nav link — "NCIt/BC Alignment" page with a Start button, two KPI cards reusing the dashboard's `.metric-card`/`.metric-progress` CSS but driven by live data, a results summary, and download links. Inline polling JS (`extra_js` block) — the first `setInterval` polling loop in this codebase (`static/js/main.js` is otherwise entirely event-delegated, one-shot `fetch` calls); kept page-scoped rather than added to the shared file for that reason. Start button styled with CDISC Blue `#134678` per `cdisc-brand-guidelines`.
- `config.py` — `ALIGNMENT_SUBMODULE_DIR`; `.gitignore` — `instance/alignment_jobs/` (per-job generated report files).
- `pyproject.toml`/`.flake8` — excluded `cdisc-bc-ncit-alignment/` from this repo's isort/black/flake8 scope (already excluded from pytest for the same reason: it's a separate git repo with its own style/line-length conventions, and running the parent's tools repo-wide was reformatting/flagging its files).
- `CLAUDE.md` — added the new blueprint/service/config rows; fixed the blueprint count (was documented as 8, actually 9 before this change, now 10 — also added the previously-undocumented `notes` blueprint).
- Wrote tests first (TDD): `tests/test_alignment_models.py`, `tests/test_alignment_runner.py`, `tests/test_ncit_alignment_routes.py` (26 new tests) — no real `subprocess` or `threading.Thread` is ever spawned; `subprocess.Popen` is mocked with canned stderr lines to test progress parsing (including comma-stripping) and the stage-1-failure-short-circuits-stage-2 case, and `threading.Thread` is mocked/replaced with a synchronous stand-in so route tests can assert final DB state with no sleeping; 472 tests passing, isort/black/flake8 clean ✅
- Verified end-to-end against a live dev server on a throwaway DB/port: the existing 14-check smoke suite still passes with the new blueprint registered; confirmed the nav link renders, the page returns 200, `/status` returns the `{"status": "none"}` shape with no prior job, and an unknown job id's download routes 404. Did not exercise a real full pipeline run (needs a live `CDISC_API_KEY` and ~3 minutes against the real NCIt/CDISC APIs) — left as a follow-up manual check.
